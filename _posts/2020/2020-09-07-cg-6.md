...
- Require primitives in object elaborated and small enough so that the triangles in object catch up with the high frequency in displacement mapping.
  - Displacement mapping only changes the vertex properties, thus if the primitive is large enough and displacement mapping is not able to change the properties inside of the triangle.
